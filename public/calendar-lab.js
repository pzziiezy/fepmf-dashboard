...
function buildProjectEvents() {
  ...
  return (state.dashboard.parents || []).map((row) => {
    ...
    const qaNames = [...new Set((qaByProject.get(row.parent.key) || []).map((item) => String(item.qaName || '').trim()).filter(Boolean))]
    return {
      id: row.parent.key,
      key: row.parent.key,
...
function renderTimeline(events) {
  ...
  const rows = events.map((item) => {
    ...
    const hasQa = item.source === 'project' && Array.isArray(item.qaNames) && item.qaNames.length
    ...
    const qaLabel = hasQa ? `QA: ${item.qaNames.join(', ')}` : ''

    return `
      <div class="planner-lab-row ${isSelected ? 'selected' : ''} ${hasQa ? 'has-qa' : ''}">
  ...
  host.innerHTML = `
    <div class="planner-lab-inspector-card ${selected.source}" ${themeStyle}>
      ...
      <div class="planner-lab-inspector-grid">
        ...
        ${selected.source === 'project' ? `<div class="full"><span>QA Plan</span><strong>${esc((selected.qaNames || []).length ? selected.qaNames.join(', ') : '-')}</strong></div>` : ''}
      </div>
...
async function loadQaAssignments() {
  try {
    const response = await fetch('/api/qa-plan')
    const data = await response.json()
    if (data.error) throw new Error(data.error)
    state.qaAssignments = Array.isArray(data.items) ? data.items : []
  } catch {
    state.qaAssignments = []
  }
}
...
async function reloadPageData() {
  await Promise.all([loadManualPlans(), loadQaAssignments()])
