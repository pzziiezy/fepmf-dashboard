...
const doneInfo=i=>i.isDone&&i.doneAt?`${thaiDT(i.doneAt)}${i.doneByEmail?` - ${i.doneByEmail}`:''}`:'-'
...
function mergeItems(){
  const todoItems=st.tasks
    ...
    .map(t=>{
      ...
      const source=(sourceType==='jira'||taskType==='jira')?'Jira':'Checklist'
...
function timelineHTML(items){
  ...
  const rows=x.map(i=>{const s=new Date(`${i.start}T00:00:00Z`),e=new Date(`${i.end}T00:00:00Z`),cs=s<a?a:s,ce=e>b?b:e;if(ce<cs)return'';const so=Math.floor((cs-a)/86400000),eo=Math.floor((ce-a)/86400000),sp=Math.max(1,eo-so+1);const isJira=i.source==='Jira';const barLabel=i.key?(isJira?`${i.key} : ${i.title}`:`${i.title} : ${i.key}`):i.title;return `<div class="planner-lab-row"><div class="planner-lab-track">${todayColInline}${dayTracks}<button class="planner-lab-bar manual" type="button" data-role="inspect" data-uid="${esc(i.uid)}" style="grid-column:${so+1} / span ${sp};--lab-bar:${esc(i.color)}"><span>${esc(barLabel)}</span></button></div></div>`}).filter(Boolean).join('')
...
async function editItem(uid){
  const it=list().find(x=>x.uid===uid);if(!it)return
  try{const a=await askAuth('open edit mode'),e=em(a?.email);if(!e)throw new Error('Jira email is required');st.editingId=it.taskId;st.editingActorEmail=e;const f=$('uniCreateForm');f.title.value=it.title||'';f.key.value=it.key||'';f.start.value=it.start||'';f.end.value=it.end||'';f.owner.value=it.owner||'';f.note.value=it.note||'';f.color.value=col(it.color||DEF_COLOR);syncColor();$('uniSaveBtn').textContent='Update item';$('uniCancelEditBtn').style.display='';notice($('uniCreateStatus'),`Editing item as ${e}...`);openCreateModal()}catch(err){notice($('uniCreateStatus'),err.message||'Unable to edit','error')}
}
...
function inspect(uid,anchor){
  ...
  const render=()=>{
    const headMain=it.source==='Jira'?(it.key||'-'):(it.title||'-')
    const headSub=it.source==='Jira'?(it.title||'-'):(it.key||'-')
    ...
    <div class="uni-pop-card" style="--uni-accent:${esc(accent)};--uni-accent-soft:${esc(accentSoft)};--uni-accent-border:${esc(accentBorder)};">
      <header class="uni-pop-head">
        <div>
          <h3 class="uni-pop-title">Edit ${esc(headMain)}</h3>
          <div class="uni-pop-key">${esc(headSub)}</div>
        </div>
    ...
    <div class="uni-pop-card" style="--uni-accent:${esc(accent)};--uni-accent-soft:${esc(accentSoft)};--uni-accent-border:${esc(accentBorder)};">
      <header class="uni-pop-head">
        <div>
          <h3 class="uni-pop-title">${esc(headMain)}</h3>
          <div class="uni-pop-key">${esc(headSub)}</div>
        </div>
        <button class="btn uni-inspector-close" id="uniInspectorClose" type="button">Close</button>
      </header>
...
function render(){toggleMonthFilter();const items=list(),done=items.filter(i=>i.isDone).length,jira=items.filter(i=>i.source==='Jira').length,checklist=items.filter(i=>i.source==='Checklist').length;notice($('uniSummary'),`${items.length} items | done ${done} | jira ${jira} | checklist ${checklist} | view ${st.view.toUpperCase()}`);let html='';if(st.view==='table')html=tableHTML(items);else if(st.view==='timeline')html=timelineHTML(items);else if(st.view==='calendar')html=calendarHTML(items);else html=cardHTML(items);$('uniViewHost').innerHTML=html;bindHost(items)}
function toggleMonthFilter(){const w=$('uniMonthWrap'),q=$('uniQuickTypeWrap'),qb=$('uniQuickTypeBox'),sw=$('uniSortWrap');const monthView=st.view==='timeline'||st.view==='calendar';const showSort=st.view!=='table';if(w)w.hidden=!monthView;if(q)q.hidden=false;if(sw)sw.hidden=!showSort;if(qb&&q?.hidden)qb.hidden=true}
